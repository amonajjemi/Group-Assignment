...
create vector and read itesm from file into it
sort vector by ISBN number
create bool variable called 'finish'
	intialize to false
	...
	WHILE(!finish)
	{
		Display Menu
			...
			SWITCH
				CASE 1: 
					User inputs ISBN number
					CALL search function and find object
						returns index of object if found, -1 if not found
					CALL cunction to calculate cost of tax on book
					Display Book's: Title, price, tax, and price + tax
					Confirm with user if the correct books was found
					cin << y for yes
					if(y or Y)
						save index of item in an int vector
					else
						return to cashier menu
					Ask user if they want to check out
					if(y or Y)
						set 'finish' to true
						Display names of books being purchased
						CALL CompleteTransaction function to get final cost
							decrement quantity of items in the file, remove item if quantity is 0
						Display the total cost of the entire purchase
					else
						Display item is saved
						return to cashier menu
					break
				CASE 2:
					Display "Exiting Module"
					return to Main Menu
	}
Write to file to edit any books that were removed from inventory
return to Main Menu
*/